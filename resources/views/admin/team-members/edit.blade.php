...
<div class="card p-6">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Edit Team Member</h3>
    </div>
    <form action="{{ route('admin.team-members.update', $teamMember) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="bio" rows="3" 
                      ...
                      placeholder="Biografi singkat">{{ old('bio', $teamMember->bio) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="social_links" 
                   value="{{ old('social_links', $teamMember->social_links ? implode(', ', $teamMember->social_links) : '') }}" 
            ...
            @if($teamMember->photo_path)
                <div class="mb-2">
                    <img src="{{ asset('storage/'.$teamMember->photo_path) }}" 
                         alt="{{ $teamMember->name }}" class="w-20 h-20 object-cover rounded-md border border-gray-200">
                </div>
            @endif
        ...
        <div class="flex justify-end">
            ...
            <button type="submit" 
                    class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
